...
- **Status:** blocked, pending Strider's decision. **Attempt 1 (sum-of-2-exponentials, Strider's sanctioned fast path) also failed GOF** — see `experiments/e3-alt-two-exp-kernel/RESULTS.md`. Same window, richer parametric kernel (fast ~5.8ms + slow ~939ms components, valid branching ratio n=0.627): KS D=0.0466, p≈0.0000, same tail-only deviation shape as the single-exponential fit. Both engines pass their own synthetic self-tests, so this is a genuine model/data mismatch, not a bug — but kernel enrichment barely moved D, which disfavors "kernel shape alone" as the explanation without confirming the competing hypothesis (a same-session attempt to test window non-stationarity via count-homogeneity chi-square was run and discarded as methodologically invalid: stationary Hawkes processes are overdispersed by construction, so that test can't discriminate the two hypotheses). **Reframed crux, flagged to Strider rather than resolved unilaterally:** D≈0.047-0.058 at n≈12-15k is practically close to passing (bulk of residuals fit almost exactly) but rejected because KS has very high power at this sample size (critical D≈0.0125). Manifesto Stubs A/B/C all assume the toolkit gets *past* N3 with a passing fit — none covers "parametric GOF fails at the pre-registered bar on substrate 1, on two kernel families." If the bar stands as written, the stub structure itself is blocked, which is evidence the *gate* (not necessarily the science) may need revisiting — a pre-registration change, the user's call.
