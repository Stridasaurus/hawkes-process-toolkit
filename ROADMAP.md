...
- **Edge-case pins (2026-07-07, second session same day — still pre-run, so still pre-registration):**
  - **E7 mixed-window result** (passes one of the two E3 windows, not both) **= fail** — matches the card's AND; no post-hoc window preference.
  - **E6 inconclusive** (preprocessing kill criterion fires) maps to the **fail row** for adjudication purposes, but is logged as *inconclusive-not-negative* — it must not be cited later as evidence the exp kernel fails on LOBSTER.
  - **Secondary metrics are descriptive only** (multi-scale subsampled-KS profile, fitted-kernel plot, E6's failure-shape comparison): the primary KS + branching-ratio bar alone adjudicates. Secondaries inform the writeup, never the outcome row.
  - **E7 adjudication fit pinned: free weights** — M free non-negative weights on the fixed log-spaced rate grid, power-law-initialized; the tied power-law profile is a nested, non-adjudicating descriptive check run only if the free fit passes (tied ⊂ free, so a free-fit fail already implies the tied fail). See the E7 card.
...
- **Assumption under test:** the E3 QQ tail excess is long-memory misfit — a long-memory kernel spanning ~1 ms to ~10 min, built as a sum of M exponentials on a fixed log-spaced rate grid, passes GOF where 1-exp and 2-exp failed.
- **Method (parametrization pinned 2026-07-07, pre-run):** rates β_1..β_M **fixed** log-spaced over ~1 ms to ~10 min (M ≈ 6–8); **M free non-negative weights, power-law-initialized** — this is the adjudicating fit. With rates fixed, λ(t) = μ + Σ_m w_m R_m(t) is linear in (μ, w), so the log-likelihood is **concave** — a single L-BFGS-B run suffices, no multi-start fragility (unlike `hawkes_core_2exp.py`, where free rates made the problem nonlinear). Per-component Markov recursion keeps the likelihood O(n·M). Same two windows as E3 (01:00–02:00 and 14:00–15:00 UTC). **If the free fit passes**, additionally fit the tied power-law profile (params μ, amplitude A, exponent ε; w_m = A·f(β_m; ε)) as a **nested, non-adjudicating descriptive check** answering "is it literally a power law?" — tied ⊂ free, so a free-fit fail already settles the tied fit.
- **Runnable check / metric:** primary (adjudicates, on the free fit) — KS p > 0.05 on rescaled inter-event times AND branching ratio in (0, 1), on **both** E3 windows; one-of-two = fail. (Convention: write components normalized, φ(t) = Σ_m w_m β_m e^{−β_m t}, so n = ∫φ = Σ_m w_m — the weights are the per-component branching contributions and the stationarity constraint is just Σw_m < 1.) Secondary (pre-registered here, not post-hoc; **descriptive only, never adjudicating**): multi-scale subsampled-KS pass fraction ≈ 0.95 at every reference n; a plot of the fitted weights over the rate grid — does it hold a power-law profile, or collapse to a few effective components?; the tied-fit result when run.
- **Timebox / cost:** ~3 h.
- **Kill criteria:** non-convergence of the adjudicating fit within the timebox → record "long-memory approximation infeasible with current machinery" and treat as *fail* in N3-alt's outcome mapping; no bespoke optimizer engineering. (Concavity makes this nearly moot for the free fit; it mainly guards the tied check, whose non-convergence just voids the descriptive extra, not the outcome.)
...
- **2026-07-07 (second session same day)** — **E6/E7 pre-registration completed and execution scheduled** (Strider, Fable session, decisions via AskUserQuestion; still pre-run, so all of this remains pre-registration). (1) **E7 parametrization pinned:** free non-negative weights on the fixed log-spaced rate grid adjudicate (concave likelihood — single-run convergence); tied power-law profile demoted to a nested, non-adjudicating descriptive check run only on a free-fit pass. Chosen over tied-adjudicates and both-must-pass because the outcome row's question is "is long memory sufficient?", not "is it literally a power law?", and the tied fit can fail for profile-shape reasons while long memory is right — a for-the-wrong-reason trigger of the fail row. (2) **Outcome-mapping edge cases pinned:** E7 mixed-window = fail; E6 inconclusive maps to the fail row but is logged inconclusive-not-negative; secondary metrics descriptive only. (3) **Execution scheduled:** E6 + E7 are the sole agenda of the next Hawkes session, on/after 2026-07-09 (after the RV presentation deadline 07-08); either order, E7 higher-information. No experiments run this session.
...
- **N3-alt:** decision made (2026-07-07) → run **E6 + E7**, outcome mapping pre-registered in the node; edge cases + E7 parametrization pinned later the same day (see log). Neither experiment has run yet — **E6 and E7 are the frontier**, scheduled as the sole agenda of the next Hawkes session on/after **2026-07-09**; either order works, E7 is the higher-information one.
